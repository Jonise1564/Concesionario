...
document.addEventListener("DOMContentLoaded", () => {
    // 🛡️ CONTROL DE ENTORNO
    if (!document.getElementById('tbodyClientes')) {
        return; 
    }

    // Inicializamos el modal de Bootstrap 5
    const modalElement = document.getElementById('modalCliente');
    if (modalElement) {
        myModalCliente = new bootstrap.Modal(modalElement);
    }

    // Inicializar catálogo de provincias
    cargarDatalistProvincias();
...
// ---------------------------------------------------------------------
// OBTENER PROVINCIAS Y CIUDADES DESDE LA BASE DE DATOS
// ---------------------------------------------------------------------
async function cargarDatalistProvincias() {
    try {
        const tokenVal = getClientesAuthToken();
...
// Recibe un parámetro para saber si viene desde una edición
window.onProvinciaChange = async function(esEdicion = false) {
    const provElem = document.getElementById('cEstadoProvincia');
    if (!provElem) return;
...
window.listarClientes = async function() {
    ...
    try {
        ...
        clientes.forEach((c, index) => {
            ...
            const cityObj = p.ciudad || p.Ciudad;
            const ciudad = (cityObj && typeof cityObj === 'object') ? (cityObj.nombre || cityObj.Nombre) : (cityObj || 'N/D');
            
            const provObj = cityObj ? (cityObj.provincia || cityObj.Provincia) : null;
            const provincia = provObj ? (provObj.nombre || provObj.Nombre || 'N/D') : (p.estadoProvincia || p.EstadoProvincia || 'N/D');
            
            const calificacion = c.calificacionCrediticia || c.CalificacionCrediticia || 'Buena';
            const fechaAltaRaw = c.fechaAlta || c.FechaAlta;
            const fechaAltaFormateada = fechaAltaRaw ? new Date(fechaAltaRaw).toLocaleDateString('es-AR') : 'N/D';
...
async function buscarClientePorId(cliente) {
    ...
    const fechaNac = p.fechaNacimiento || p.FechaNacimiento;
    if (fechaNac && document.getElementById('cFechaNacimiento')) {
        document.getElementById('cFechaNacimiento').value = fechaNac.split('T')[0];
    }
    ...
    if (cityObj && typeof cityObj === 'object') {
        ciuActual = cityObj.nombre || cityObj.Nombre || '';
        const provObj = cityObj.provincia || cityObj.Provincia;
        provActual = (provObj && typeof provObj === 'object') ? (provObj.nombre || provObj.Nombre) : (p.estadoProvincia || p.EstadoProvincia || '');
    }
    ...
    // ⏳ CLAVE: Esperamos de verdad a que cargue las ciudades antes de asignarla
    await window.onProvinciaChange(true); 
...
// ---------------------------------------------------------------------
// ENVIAR DATOS AL BACKEND (REESTRUCTURADO A JSON)
// ---------------------------------------------------------------------
window.guardarCliente = async function() {
    const form = document.getElementById('formCliente');
    if (!form) return;
    ...
    const idCliente = parseInt(document.getElementById('cId')?.value || "0");
    const idPersona = parseInt(document.getElementById('cIdPersonaId')?.value || "0");

    const inputCiudad = document.getElementById('cCiudad');
    const textoCiudadEscrita = inputCiudad ? inputCiudad.value.trim().toLowerCase() : '';
    let ciudadIdResuelto = 1; // Fallback por defecto si no matchea
    ...
    // ✨ Construimos un JSON estructurado para tu API en C#
    const payload = {
        Id: idCliente,
        IdPersonaId: idPersona,
        CalificacionCrediticia: document.getElementById('cCalificacionCrediticia')?.value || 'Buena',
        Observaciones: document.getElementById('cObservaciones')?.value || '',
        Persona: {
            Id: idPersona,
            DocumentoIdentidad: document.getElementById('cDocumentoIdentidad')?.value || '',
            ...
            TelefonoAlternativo: document.getElementById('cTelefonoAlternativo')?.value || '',
            FechaNacimiento: document.getElementById('cFechaNacimiento')?.value || null,
            ...
            CodigoPostal: document.getElementById('cCodigoPostal')?.value || '',
            Pais: document.getElementById('cPais')?.value || 'Argentina',
            CiudadId: parseInt(ciudadIdResuelto)
        }
    };
    ...
    try {
        ...
        const response = await fetch('/api/Clientes/Guardar', {
            ...
            headers: { 
                'Authorization': `Bearer ${tokenVal}`,
                'Content-Type': 'application/json' // Avisamos al backend que va un JSON
            },
        ...
        alert("Error de red: No se pudo establecer contacto con el servidor.");
    }
}
